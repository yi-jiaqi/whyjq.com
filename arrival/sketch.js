...
let soundPlaceholder;

function preload() {
    // Placeholder for sound, without loading a real file
...
function setup() {
    ...
    */
    ...
    humanNote.style('font-size', '18px');
    ...
    orMark.style("font-size", "12px");
    orMark.style("color", "FF0909");
    orMark.style('text-align', 'center');
    ...
    textBox.style("font-size", "15px");
    textBox.style("color", "FF0909");
...
function displayAoa() {
    ...
    for (let i = 0; i < graphicsIndex; i++) {
        ...
        collectionMarksAlien[i].style('font-size', '13px');
        ...
        collectionMarksHuman[i].style('font-size', '14px');
...
function passiveTalk() {
    ...
    humanNote.html("Example question:  **" + humanAskInString + "**  You may also ask:");
    let humanNoteWidth = textWidth(humanNote.html())
    humanNote.position(width / 2 - humanNoteWidth / humanNoteWidthMovement, height / 1.08 - 100);
    trigger();
}
...
async function trigger() {
    ...
    try {
        chatGPTanswerArray = chatGPTanswerOriginal;
        console.log(chatGPTanswerOriginal);
    } catch (error) {
        chatGPTanswerArray = ['(Unable to translate)'
        ]
    }
    ...
    alienAnswerInString = chatGPTanswerArray;
...
async function sendAndDisplayGPTResponse() {
    const message = humanAskInString;

    // Replace 'apiUrl' with your actual API Gateway Invoke URL
    const apiUrl = 'https://5eq4cvv13i.execute-api.us-east-2.amazonaws.com/dev/translation'; // Use your API Gateway Invoke URL here
    ...
    try {
        ...
        const response = await fetch(apiUrl, {
            ...
            body: JSON.stringify({ message })  // Sending the message as a JSON object
        });
        ...
        console.log('Parsed API reply:', reply);

        // Parse the stringified body from the API response
        const parsedBody = JSON.parse(reply.body);

        // Ensure the parsed body has the message property
        if (!parsedBody.message) {
            throw new Error('Invalid response format: Missing "message" field.');
        }

        // Set the response to the variable and display it on the page
        chatGPTanswerOriginal = parsedBody.message;

        const responseElement = document.querySelector('#chatGPTResponse');
        if (responseElement) {
            responseElement.textContent = chatGPTanswerOriginal;
        } else {
            console.error('Element #chatGPTResponse not found in the DOM.');
        }
        ...
        if (error.name === 'TypeError') {
            console.error('Network-related or CORS issue, check if API Gateway URL is correct or CORS is enabled.');
            document.querySelector('#chatGPTResponse').textContent = 'Network error. Please check your connection or try again later.';
        } else if (error.message.includes('400')) {
            console.error('Bad request: likely an issue with the request body or formatting.');
            document.querySelector('#chatGPTResponse').textContent = 'Invalid input. Please check your message and try again.';
        } else if (error.message.includes('500')) {
            console.error('Server error: there may be an issue with the Lambda function or API Gateway.');
            document.querySelector('#chatGPTResponse').textContent = 'Server error. Please try again later.';
        } else {
            console.error('Unexpected error occurred.');
            document.querySelector('#chatGPTResponse').textContent = 'Unexpected error. Please try again later.';
        }
    }
}